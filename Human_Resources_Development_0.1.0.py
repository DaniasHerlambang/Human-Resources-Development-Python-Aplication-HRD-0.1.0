...
import ttk
import time 
import tkColorChooser #MEMANGGIL DIALOG WARNA
...
class Data(Frame):
    ...
    def initUI(self):
        self.update()
        self.teksJam = StringVar()
        self.datJam_menu = time.strftime("PT.MULTINASIONAL - %A. %d %B %Y ",
                                           time.localtime())
        # atur ukuran window
        # menempatkan window di tengah layar PC/Laptop , geometry diatur disini
        lebar = 400
        tinggi = 100
        ...
        # Label atas untuk selamat datang
        
        self.Lbl_atas = Label(mainFrame,text='SELAMAT DATANG',font=('Century', 30),bg='black',fg='white')
        self.Lbl_atas.pack(expand=YES,fill=BOTH)
        
        # ************************* komponen pada menubar. import , tampilkan
        fileMenu = Menu(self.menubar, tearoff=0)
        self.menubar.add_cascade(label = 'IMPORT',command=self.impor) 
        fileMenu = Menu(self.menubar, tearoff=0)
        self.menubar.add_cascade(label="DATA KOSONG",command=self.data1)

        # frame bawah untuk keluar dan jam
        
        self.fr_bawah = Frame (mainFrame,bg='black')
        self.fr_bawah.pack(side=BOTTOM,fill=BOTH,expand=YES)

        # Button untuk keluar 
        self.tmbl_keluar = Button(self.fr_bawah, text='KELUAR', command=self.keluar,
                                  font=('Century', 10),bg='black', fg='white')
        self.tmbl_keluar.pack(side=LEFT)

        # Label untuk keluar 
        self.jam = Label(self.fr_bawah,bg='black',textvariable=self.teksJam
                                 ,compound='center',fg='lavender',font=('Century', 13))
        self.jam.pack(side=RIGHT)
      
    def keluar(self):
        # Ketika keluar title parent akan berubah tulisan
        self.parent.title('PT.MULTINASIONAL - EXIT YOUR SYSTEM')
        self.Lbl_atas.configure(text='TERIMA KASIH')

        # Ketika keluar akan keluar prosesbar dan window akan menutup setelah 5 detik
        s = ttk.Style()
        s.theme_use('classic')
        ...
        pb_hd.start(4)
        root.after(4000, root.destroy)  #menutup window setelah 3 detik
...
#************************************ fungsi untuk lampu kelap-kelip di selamat datang *******************************************************
#---- dengan configure label dan tombol setiap mili detik lalu di jalankan dengan waktu (update) akan terlihat kemerlapan seperti lampu ------
    def lampu(self, a):
        for i in range(a):
            self.parent.update()
            time.sleep(0.02)
            self.Lbl_atas.configure(bg='lavender')
            
    def lampu1(self, a):
        for i in range(a):
            self.parent.update()
            time.sleep(0.04)
            self.Lbl_atas.configure(bg='white')
            self.tmbl_keluar.configure(fg='black')
    ...
    def lampu3(self, a):
        for i in range(a):
            self.parent.update()
            time.sleep(0.04)
            self.Lbl_atas.configure(bg='gray')
            self.tmbl_keluar.configure(fg='white')

    def lampu4(self, a):
        for i in range(a):
            self.parent.update()
            time.sleep(0.04)
            self.Lbl_atas.configure(bg='black')
    ...
    def flashing(self):
        ...
        self.Lbl_atas.after(50, self.flashing)
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    def impor(self):
        try:
            ...
            #******************************dimasukan ke data_mhs1(kosong) untuk data bawah*****************************************************
            p1=self.list1
            self.data_hrd_B =[]
...
#--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------#
    ...
    def data1(self):
        try:
            ...
            self.Lbl_atas.configure(text='SELAMAT DATANG')
...
#****************************************** FUNGSI TAMBAH DATA *************************************************************************************************             
                  ...
                  def tambah(self):
                     ...
                     self.entryKosong() #entry dikosongkan supaya pengguna bisa memasukan data
                  ...
                  #jika ada salah satu dari entry kosong akan keluar peringatan
                    ...
                    if tambahanNam == ''\
                       ...
                       or tambahanKd == '':
                        ...
                        #*****************data_hrd_A di append lalu menambahkan di GUI******************************
                        self.entryKosong()
                        ...
                        #*****************self.data_hrd_B di append lalu menambahkan di GUI******************************
...
#****************************************** FUNGSI MENGHAPUS SEMUA DATA ******************************************************************************************             
                  ...
                  def hapus(self):
                            #disini bukan asli menghapus tetapi menuliskan kembali judul pada csv secara otomatis dengan (w / write)
                            #secara otomatis menghapus semua dan menulis judul seolah-olah datanya dihapus kecuali judul
                            #setelah dihapus akan keluar pemberitahuan , dan window data mati setelah 3 detik
                        ...
                        if askyesno("PT.MULTINASIONAL","ANDA YAKIN INGIN MENGHAPUS SEMUA ?"):
                            if askyesno("PT.MULTINASIONAL","APAKAH SUDAH DISETUJUI BOS BESAR ?"):
                                if askyesnocancel("PT.MULTINASIONAL","ANDA YAKIN BETUL INGIN MENGHAPUS SEMUA ?"):
                                    ...
                                    #*****************self.data_hrd_B di append lalu ditampilkan di data bawah*****************
                                    self.data_hrd_B=[]
